...
void TH_Rotation_Elimination_AVX(int k, int t_id) {
	float temp;  // 设计思想同C_GE
	__m256 va, vt, vaik, vakj, vaij, vx;
	int j = 0;
	for (int i = k + t_id + 1; i < n; i += THREAD_NUM)  // 执行本线程对应的任务，c代表任务计数
...
void Thread_Dynamic_Rotation_GE_AVX(int n) {  // Thread优化的高斯消去算法，动态，轮流划分任务
	int tasknum = 0;
	thread* t = new thread[THREAD_NUM];
	float t1, t2;  // 使用两个浮点数暂存数据以减少程序中地址的访问次数
	__m256 va, vt;
	for (int k = 0; k < n; k++)
	{
		...
		for (int i = 0; i < THREAD_NUM; i++)
		{
			t[i] = thread(&TH_Rotation_Elimination_AVX, k, i);
			//cout << t[i].get_id() << endl;
		}
		for (int i = 0; i < THREAD_NUM; i++) {
			t[i].join();
...
int main() {
	...
	cout << "PThread_Dynamic_Block_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	cout << "PThread_Dynamic_Rotation_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	cout << "PThread_Static_OnlyElim_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	cout << "PThread_Static_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	cout << "PThread_Static_Barrier_Rotation_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	cout << "PThread_Static_Barrier_Block_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	Thread_Dynamic_Rotation_GE_AVX(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "Thread_Dynamic_Rotation_GE_AVX: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
