...
void TH_Rotation_Elimination(int k, int t_id) {
	float temp;  // 设计思想同C_GE
	for (int i = k + t_id + 1; i < n; i += THREAD_NUM)  // 执行本线程对应的任务，c代表任务计数
	{
		temp = mat[i][k];
...
void* PT_Block_Elimination_Neon(void* param) {
	PT_EliminationParam* tt = (PT_EliminationParam*)param;
	int k = tt->k;
	int t_id = tt->t_id;
	int tasknum = tt->tasknum;
	...
	if(t_id != THREAD_NUM - 1){
		for (int c = 0; c < tasknum; i++, c++)  // 执行本线程对应的任务，c代表任务计数
		{
			...
			for (j = k + 1; j + 4 < n; j += 4)
			{
				...
				vst1q_f32((float32_t*)&mat[i][j], vaij);
			}
			for (j; j < n; j++)
			{
				mat[i][j] -= temp * mat[k][j];
...
void* PT_Rotation_Elimination_Neon(void* param) {
	PT_EliminationParam* tt = (PT_EliminationParam*)param;
	int k = tt->k;
	int t_id = tt->t_id;
	float temp;  // 设计思想同C_GE
...
void* PT_Static_Elimination_Neon(void* param) {
	PT_StaticParam* p = (PT_StaticParam*)param;
	int t_id = p->t_id;
	float t2;  // 使用浮点数暂存数据以减少程序中地址的访问次数
	float32x4_t vaik, vakj, vaij, vx;
...
void* PT_Static_Div_Elem_Neon(void* param) {  // 三重循环全部纳入
	PT_StaticParam* p = (PT_StaticParam*)param;
	int t_id = p->t_id;
	float t1, t2;  // 使用浮点数暂存数据以减少程序中地址的访问次数
	...
	for (int k = 0; k < n; ++k) {
		...
		if (t_id == 0)
		{
			vt = vmovq_n_f32(mat[k][k]);  // 加载四个重复值到vt中
			t1 = mat[k][k];
			for (j = k + 1; j + 4 < n; j += 4)
			{
				va = vld1q_f32(&mat[k][j]);
				va = vdivq_f32(va, vt);
				vst1q_f32((float32_t*)&mat[k][j], va);
			}
			for (j; j < n; j++)
			{
				mat[k][j] = mat[k][j] / mat[k][k];  // 善后
			}
			mat[k][k] = 1.0;
		}
		else {
...
void* PT_Static_Barrier_Div_Elem_Neon(void* param) {
	PT_StaticParam* p = (PT_StaticParam*)param;
	int t_id = p->t_id;
	float t1, t2;  // 使用浮点数暂存数据以减少程序中地址的访问次数
	float32x4_t va, vt, vaik, vakj, vaij, vx;
...
void* PT_Static_Barrier_Div_Elem_Block_Neon(void* param) {
	PT_StaticParam* p = (PT_StaticParam*)param;
	int t_id = p -> t_id;
	float t1, t2;  // 使用浮点数暂存数据以减少程序中地址的访问次数
	int tasknum = 0;
...
void TH_Rotation_Elimination_Neon(int k, int t_id) {
	float temp;  // 设计思想同C_GE
	float32x4_t va, vt, vaik, vakj, vaij, vx;
	int j = 0;
	for (int i = k + t_id + 1; i < n; i += THREAD_NUM)  // 执行本线程对应的任务，c代表任务计数
...
void Thread_Dynamic_Rotation_GE(int n) {  // Thread优化的高斯消去算法，动态，轮流划分任务
	int tasknum = 0;
	...
	for (int k = 0; k < n; k++)
	{
		...
		for (int i = 0; i < THREAD_NUM; i++)
		{
			t[i] = thread(&TH_Rotation_Elimination, k, i);
			//cout << t[i].get_id() << endl;
		}
		for (int i = 0; i < THREAD_NUM; i++) {
			t[i].join();
		}
...
void PThread_Dynamic_Block_GE_Neon(int n) {  // PThread优化的高斯消去算法，动态，按块划分任务
	int tasknum = 0;
	pthread_t* handles = (pthread_t*)malloc((THREAD_NUM) * sizeof(pthread_t));  // 为线程句柄分配内存空间
	PT_EliminationParam* param = (PT_EliminationParam*)malloc((THREAD_NUM) * sizeof(PT_EliminationParam));  // 存储线程参数
	float t1, t2;  // 使用两个浮点数暂存数据以减少程序中地址的访问次数
	...
	for (int k = 0; k < n; k++)
	{
		...
		if (tasknum < THREAD_NUM)  // 如果一个线程的任务数量还不如线程数量多，那么就直接采用主线程
		{
			...
			//创建线程
			for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
				pthread_create(&handles[t_id], nullptr, PT_Block_Elimination_Neon, (void*)&param[t_id]);
			}
			for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
				pthread_join(handles[t_id], nullptr);
			}
...
void PThread_Dynamic_Rotation_GE_Neon(int n) {  // PThread优化的高斯消去算法，动态，轮流划分任务
	int tasknum = 0;
	pthread_t* handles = (pthread_t*)malloc(THREAD_NUM * sizeof(pthread_t));  // 为线程句柄分配内存空间
	PT_EliminationParam* param = (PT_EliminationParam*)malloc(THREAD_NUM * sizeof(PT_EliminationParam));  // 存储线程参数
	float32x4_t va, vt, vaik, vakj, vaij;
	...
	for (int k = 0; k < n; k++)
	{
		...
		if (tasknum < THREAD_NUM)  // 如果一个线程的任务数量还不如线程数量多，那么就直接采用主线程
		{
			...
			//创建线程
			for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
				pthread_create(&handles[t_id], nullptr, PT_Rotation_Elimination_Neon, (void*)&param[t_id]);
			}
			for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
				pthread_join(handles[t_id], nullptr);
			}
...
void PThread_Static_OnlyElim_GE_Neon(int n) {  // PThread优化的高斯消去算法，静态，只将三重循环中的后二重纳入线程
	//初始化信号量
	sem_init(&sem_main, 0, 0);
	for (int i = 0; i < THREAD_NUM; i++) {
		sem_init(&sem_workerstart[i], 0, 0);
	...
	for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
		param[t_id].t_id = t_id;
		pthread_create(&handles[t_id], nullptr, PT_Static_Elimination_Neon, (void*)&param[t_id]);
	}
	for (int k = 0; k < n; k++) {
		//主线程做除法操作
		int j = 0;
...
void PThread_Static_GE_Neon(int n) {  // PThread优化的高斯消去算法，静态，将三重循环全部纳入线程
	//初始化信号量
	sem_init(&sem_leader, 0, 0);
	for (int i = 0; i < THREAD_NUM - 1; ++i) {
		sem_init(&sem_Divsion[i], 0, 0);
	...
	for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
		param[t_id].t_id = t_id;
		pthread_create(&handles[t_id], nullptr, PT_Static_Div_Elem_Neon, (void*)&param[t_id]);
	}
	for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
		pthread_join(handles[t_id], nullptr);
	}
...
void PThread_Static_Barrier_GE_Neon(int n) {
	//初始化 barrier
	pthread_barrier_init(&barrier_Divsion, nullptr, THREAD_NUM);
	pthread_barrier_init(&barrier_Elimination, nullptr, THREAD_NUM);
	//创建线程
	...
	for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
		param[t_id].t_id = t_id;
		pthread_create(&handles[t_id], nullptr, PT_Static_Barrier_Div_Elem_Neon, (void*)&param[t_id]);
	}
...
void PThread_Static_Barrier_Block_GE_Neon(int n) {
	//初始化 barrier
	pthread_barrier_init(&barrier_Divsion, nullptr, THREAD_NUM);
	pthread_barrier_init(&barrier_Elimination, nullptr, THREAD_NUM);
	//创建线程
	...
	for (int t_id = 0; t_id < THREAD_NUM; t_id++) {
		param[t_id].t_id = t_id;
		pthread_create(&handles[t_id], nullptr, PT_Static_Barrier_Div_Elem_Block_Neon, (void*)&param[t_id]);
	}
...
void Thread_Dynamic_Rotation_GE_Neon(int n) {  // Thread优化的高斯消去算法，动态，轮流划分任务
	int tasknum = 0;
	thread* t = new thread[THREAD_NUM];
	float t1, t2;  // 使用两个浮点数暂存数据以减少程序中地址的访问次数
	float32x4_t va, vt, vaik, vakj, vaij;
	...
	for (int k = 0; k < n; k++)
	{
		...
		for (int i = 0; i < THREAD_NUM; i++)
		{
			t[i] = thread(&TH_Rotation_Elimination_Neon, k, i);
			//cout << t[i].get_id() << endl;
		}
		for (int i = 0; i < THREAD_NUM; i++) {
			t[i].join();
...
float** generate(int n) {
	ifstream inp("/home/s2012411/test/PThread/input.txt");
	inp >> n;
	float** m = new float* [n];
	for (int i = 0; i < n; i++)
	{
...
int main() {
	...
	//float** m10 = generate(n);
	//float** m10 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	Thread_Dynamic_Rotation_GE(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "Thread_Dynamic_Rotation_GE: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);

	//-----------------------------------------------------------------

	//float** m11 = generate(n);
	//float** m11 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Dynamic_Block_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Dynamic_Block_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);

	//-----------------------------------------------------------------

	//float** m12 = generate(n);
	//float** m12 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Dynamic_Rotation_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Dynamic_Rotation_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);

	//-----------------------------------------------------------------

	//float** m13 = generate(n);
	//float** m13 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Static_OnlyElim_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Static_OnlyElim_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);

	//-----------------------------------------------------------------

	//float** m14 = generate(n);
	//float** m14 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);

	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Static_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Static_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);

	//-----------------------------------------------------------------

	//float** m15 = generate(n);
	//float** m15= aligned_generate(n);
	global_generate(n);
	//show(mat, n);

	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Static_Barrier_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Static_Barrier_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	PThread_Static_Barrier_Block_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "PThread_Static_Barrier_Block_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
	...
	//float** m16 = generate(n);
	//float** m16 = aligned_generate(n);
	global_generate(n);
	//show(mat, n);

	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	Thread_Dynamic_Rotation_GE_Neon(n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒

	cout << "Thread_Dynamic_Rotation_GE_Neon: " << time_use / 1000
		<< "ms" << endl;
	//show(mat, n);
